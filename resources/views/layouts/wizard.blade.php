...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" id="htmltag" class="">
    ...
    <body x-data="wizard_layout" class="bg-light dark:bg-dark-1 transition duration-300">
        ...
        @livewireScripts
        <!-- Header -->
            @livewire('ui.header-wizard', ['page' => $page])
        <!-- Header -->

        <div class="font-robo text-darker dark:text-light antialiased min-h-screen w-full transition duration-300">    
            {{ $slot }}
        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('wizard_layout', () => ({
                modals: false,
                focused_content_type: 'video',
    ...
    <script>
        window.addEventListener('load', (event) => {
            ...
            function timeFunction() {
                setTimeout(function(){ document.getElementById('loading_page').classList.add('hidden'); }, 1000);
            }
        });
    </script>
