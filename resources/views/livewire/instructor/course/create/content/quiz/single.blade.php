    ...
    <div class="w-full relative overflow-hidden transition-all pl-4 max-h-0 duration-300" style="" x-ref="container{{ $question_id }}" :style="focused_question == {{ $question_id }} ? 'max-height: ' + $refs.container{{ $question_id }}.scrollHeight + 'px' : ''">
        <div class="w-full h-auto" :class="{'pb-10': answerKey}">
            <div>
                ...
                <div class="group relative flex justify-between items-center z-40" :class="{'hover:bg-washed-primary': answerKey}">
                    ...
                    <div class="flex w-full items-center justify-between">
                        <div class="flex items-center w-full">
                            ...
                            @livewire('instructor.course.create.content.quiz.answer-single', ['answer_id' => $answer->id], key($answer->id))
                        </div>
                    </div>
                    <button x-show="!answerKey" @click="removeAnswer({{ $answer->id }})" class="hidden group-hover:flex w-8 h-8 rounded-full hover:text-red-500"><i class="fa-solid fa-circle-xmark"></i></button>
                </div>
                @endforeach
                <div class="flex">                    
